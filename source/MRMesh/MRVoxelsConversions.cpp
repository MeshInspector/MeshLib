...
std::optional<SimpleVolume> meshToSimpleVolume( const Mesh& mesh, const MeshToSimpleVolumeParams& params /*= {} */ )
{
        ...
        [&] ( const tbb::blocked_range<size_t>& range )
    {
        for ( size_t i = range.begin(); i < range.end(); ++i )
        {
            ...
            {
                dist = fwn.calc( voxelCenter, FLT_MAX );
                //if ( fwn.calc( voxelCenter, FLT_MAX ) > 0.5f )
                //    dist = -dist;
                //if ( fwn.calc( voxelCenter, 3.0f ) > 0.5f )
                //    dist = -dist;
                //dist = ;
                //int numInters = 0;
...
std::optional<Mesh> simpleVolumeToMesh( const SimpleVolume& volume, const Mesh& refMesh, const VdbVolumeToMeshParams& params /*= {} */ )
{
    ...
    auto setupSeparation = [&] ( size_t base, NeighborDir dir )->SeparationPoint
    {
        ...
        if ( basePos[dir] + 1 >= volume.dims[dir] )
            return {};
        auto coord = basePos;// openvdb::Coord{ basePos.x + minCoord.x(),basePos.y + minCoord.y(),basePos.z + minCoord.z() };
        const auto& valueB = volume.data[indexer.toVoxelId( coord )];// acc.getValue( coord );// volume.data[base];
        auto nextCoord = coord;
        nextCoord[dir] += 1;
        const auto& valueD = volume.data[indexer.toVoxelId( nextCoord )];// acc.getValue( nextCoord );// volume.data[nextId];
        bool bLower = valueB < params.iso;
        bool dLower = valueD < params.iso;
        ...
        SeparationPoint res;
        res.low = bLower;
        Vector3f coordF = Vector3f( coord );
        Vector3f nextCoordF = Vector3f( nextCoord );
        auto bPos = params.basis.b + params.basis.A * coordF + 0.5f * volume.voxelSize;
        auto dPos = params.basis.b + params.basis.A * nextCoordF + 0.5f * volume.voxelSize;
        //float prevVal = params.iso;

        ratio = std::clamp( std::abs( params.iso - baseVal ) / std::abs( dirVal - baseVal ), 0.0f, 1.0f );
        res.position = ( 1.0f - ratio ) * bPos + ratio * dPos;
        //for ( int numIter = 0; numIter < 15; ++numIter )
        //{
        //    auto val = fwn.calc( res.position, 10.0f );
        //    if ( val == prevVal )
        //        break;
        //    prevVal = val;
        //    if ( std::abs( val - params.iso ) < 1e-5f )
        //        break;
        //    if ( bLower == ( val < params.iso ) )
        //    {
        //        baseVal = val;
        //        bPos = res.position;
        //    }
    ...
    tbb::parallel_for( tbb::blocked_range<size_t>( 0, indexer.size() ), [&] ( const tbb::blocked_range<size_t>& range )
    {
        ...
        for ( size_t ind = range.begin(); ind < range.end(); ++ind )
        {
            ...
            {
                auto pos = basePos + cVoxelNeighbors[i];
                if ( volume.data[indexer.toVoxelId( pos )] >= params.iso )
                    continue;
                voxelConfiguration |= ( 1 << cMapNeighborsShift[i] );
            }
