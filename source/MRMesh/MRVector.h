...
public:
    ...
    Vector() = default;
    explicit Vector( size_t size ) requires std::default_initializable<T> : vec_( size ) { }
    ...
    [[nodiscard]] bool operator == ( const Vector & b ) const requires std::equality_comparable<T> { return vec_ == b.vec_; }
    [[nodiscard]] bool operator != ( const Vector & b ) const requires std::equality_comparable<T> { return vec_ != b.vec_; }
    ...
    void resize( size_t newSize ) requires std::movable<T> && std::default_initializable<T> { vec_.resize( newSize ); }
    void resize( size_t newSize, const T & t ) requires std::movable<T> { vec_.resize( newSize, t ); }

    // resizes the vector skipping initialization of its elements (more precisely initializing them using ( noInit ) constructor )
    void resizeNoInit( size_t targetSize ) requires std::constructible_from<T, NoInit>
    {
    ...
    [[nodiscard]] reference autoResizeAt( I i ) requires std::default_initializable<T>
    {
        if ( i + 1 > size() )
