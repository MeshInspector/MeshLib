...
# param 1 root build folder (i.e. ./build/Debug)
buildFolder=$1
...
compiler=`echo $4 | sed "s/ /_/"`

mkdir -p time_log
for file in `find $buildFolder/*/ -name compile_timings.txt`
do
    echo "collect $file"
    cat $file >> time_log/compile_timings_${system}_${config}_${compiler}.csv
done
for file in `find $buildFolder/*/ -name link_timings.txt`
do
    echo "collect $file"
