...
void ImGuiMenu::draw_object_names()
{
    ...
    const ImVec2
        ...
        paddingA = ImGuiMath::round( ImVec2( 4, 2 ) * menuScaling ),
        // Bottom-right text padding.
        paddingB = ImGuiMath::round( ImVec2( 4, 4 ) * menuScaling );

    const float
        // Button rounding.
        rounding = 4 * menuScaling,
        ...
        buttonSpacingToPoint = 8 * menuScaling,
        lineWidth = 2 * menuScaling,
        lineOutlineWidth = 2 * menuScaling,
        buttonOutlineWidth = 2 * menuScaling;

    const bool isDark = ColorTheme::getPreset() == ColorTheme::Preset::Dark;

    const ImU32
        colorOutline = ImGui::ColorConvertFloat4ToU32( isDark ? ImVec4( 0, 0, 0, 0.5f ) : ImVec4( 1, 1, 1, 0.5f ) );

    const Viewer& viewer = getViewerInstance();

    ImDrawList& drawList = *ImGui::GetBackgroundDrawList();

    ImGui::PushStyleVar( ImGuiStyleVar_WindowMinSize, ImVec2{} );
    ImGui::PushStyleVar( ImGuiStyleVar_WindowPadding, ImVec2{} );
    MR_FINALLY{ ImGui::PopStyleVar( 2 ); };
    ImGui::PushStyleColor( ImGuiCol_Border, ImVec4{} );
    ImGui::PushStyleColor( ImGuiCol_BorderShadow, ImVec4{} );
    MR_FINALLY{ ImGui::PopStyleColor( 2 ); };

    clickableNames_.clear();

    bool anyNameHovered = false;

    auto lambda = [&]( auto& lambda, const std::shared_ptr<Object>& object, ViewportMask visibilityMask ) -> void
    {
        visibilityMask &= object->visibilityMask();
        if ( visibilityMask.empty() )
            return;

        if ( auto visual = dynamic_cast<const VisualObject*>( object.get() ); visual && visual->showName() )
        {
            auto xf = visual->worldXf();

            const NameTagParams& nameParams = visual->nameTagParams();

            Vector3f worldPoint = xf( nameParams.point );
            Vector3f worldPoint2 = xf( nameParams.point + nameParams.localOffset );
            ImVec2 point3Offset = ImVec2( nameParams.screenOffset ) * menuScaling;

            const auto& text = visual->name();
            ImVec2 textSize = ImGui::CalcTextSize( text.c_str() );

            ImVec4 colorMainFloat = ImVec4( visual->getFrontColor( visual->isSelected() ) );
            ...
            ImVec4 colorHoveredFloat = ImVec4( visual->getFrontColor( visual->isSelected() ) );
            ...
            for ( ViewportId viewportId : visibilityMask )
            {
                if ( !visual->getVisualizeProperty( unsigned( VisualizeMaskType::Name ), viewportId ) )
                    continue; // The name is hidden in this viewport.

                const auto& viewport = viewer.viewport( viewportId );

                auto clipRect = viewport.getViewportRect();
                clipRect.min.y = ImGui::GetIO().DisplaySize.y - clipRect.min.y;
                clipRect.max.y = ImGui::GetIO().DisplaySize.y - clipRect.max.y;
                std::swap( clipRect.min.y, clipRect.max.y );

                drawList.PushClipRect( clipRect.min, clipRect.max );
                MR_FINALLY{ drawList.PopClipRect(); };

                auto toScreenCoords = [&]( Vector3f point ) -> ImVec2
                {
                    auto result = viewport.projectToViewportSpace( point );
                    return ImVec2( result.x, result.y ) + ImVec2( clipRect.min );
                };

                Vector3f fixedWorldPoint = worldPoint;
                Vector3f fixedWorldPoint2 = worldPoint2;

                if ( nameParams.rotateToScreenPlaneAroundSphereCenter )
                {
                    Vector3f center = xf( *nameParams.rotateToScreenPlaneAroundSphereCenter );
                    ...
                    if ( deltaLen > 0 )
                    {
                        ...
                        Vector3f dirTowardsCamera = viewport.getViewXf().A.z.normalized();
                ...
                ImVec2 point = toScreenCoords( fixedWorldPoint );
                ImVec2 point2 = toScreenCoords( fixedWorldPoint2 );

                if ( nameParams.rotateLocalOffset90Degrees )
                    point2 = point + ImGuiMath::rot90( point2 - point );

                ImVec2 point3 = point2 + point3Offset;

                ImVec2 lastSegmentOffset;
                if ( point3Offset != ImVec2{} )
                    lastSegmentOffset = point3Offset;
                else if ( ImVec2 d = point3 - point; d != ImVec2{} )
                    lastSegmentOffset = d;

                ImVec2 textCenter = point3;
                ImVec2 textPos = textCenter - textSize / 2;

                if ( lastSegmentOffset != ImVec2{} )
                {
                    ImVec2 prevPoint = point3 - lastSegmentOffset;

                    ImVec2 pointA = textPos - paddingA - buttonSpacingToPoint;
                    ImVec2 pointB = textPos + textSize + paddingB + buttonSpacingToPoint;
                ...
                textPos = ImGuiMath::round( textPos );

                bool isHovered = false;
                bool isActive = false;

                // The clickable region, if enabled.
                if ( clickingObjectNameSelectsObject && !anyNameHovered )
                {
                    // Clip window size to the viewport:

                    ImVec2 windowCornerA = ImGuiMath::clamp( textPos - paddingA, ImVec2( clipRect.min ), ImVec2( clipRect.max ) );
                    ImVec2 windowCornerB = ImGuiMath::clamp( textPos + textSize + paddingB, ImVec2( clipRect.min ), ImVec2( clipRect.max ) );

                    // If it wasn't clipped to nothing...
                    if ( ImGuiMath::CompareAll( windowCornerA ) < windowCornerB )
                    {
                        clickableNames_.push_back( { .a = windowCornerA, .b = windowCornerB, .object = object } );
        ...
        for ( const auto& child : object->children() )
            lambda( lambda, child, visibilityMask );
    };
    lambda( lambda, SceneRoot::getSharedPtr(), viewer.getPresentViewports() );
}
