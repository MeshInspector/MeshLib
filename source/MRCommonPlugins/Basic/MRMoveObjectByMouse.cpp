...
void MoveObjectByMouse::drawDialog( float menuScaling, ImGuiContext*)
{
    ...
    ImGui::Text( "Mode:" );
    UI::radioButtonOrModifier( "Move",   moveByMouse_.modXfMode, int( XfMode::Move ),   0,                          UI::ImGuiModCtrlOrSupper() | ImGuiMod_Alt );
    ImGui::SameLine();
    UI::radioButtonOrModifier( "Rotate", moveByMouse_.modXfMode, int( XfMode::Rotate ), UI::ImGuiModCtrlOrSupper(), UI::ImGuiModCtrlOrSupper() | ImGuiMod_Alt );
    ImGui::SameLine();
    UI::radioButtonOrModifier( "Scale",  moveByMouse_.modXfMode, int( XfMode::Scale ),  ImGuiMod_Alt,               UI::ImGuiModCtrlOrSupper() | ImGuiMod_Alt );
...
{
    if ( ( modifiers & ~( GLFW_MOD_SHIFT | glfwModCtrlOrSupper() | GLFW_MOD_ALT ) ) != 0 ||
     ( modifiers & ( glfwModCtrlOrSupper() | GLFW_MOD_ALT ) ) == ( glfwModCtrlOrSupper() | GLFW_MOD_ALT ) )
    ...
    else if ( int( modXfMode ) == int( XfMode::Rotate ) || ( modifiers & glfwModCtrlOrSupper() ) == glfwModCtrlOrSupper() )
