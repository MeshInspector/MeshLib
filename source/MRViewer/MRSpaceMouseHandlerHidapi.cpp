...
void SpaceMouseHandlerHidapi::startUpdateThread_()
{
    ...
    updateThread_ = std::thread( [&]() {
        do {
            ...
            do {
                // hid_read() waits (blocks) until there is data to read before returning
                packet_length = hid_read(device_, packet.data(), packet.size());
            } while (packet_length <= 0); // wait for the first valid package
...
void SpaceMouseHandlerHidapi::findDevice( struct hid_device_info *cur_dev, HidDevice& attachDevice )
{
    while( cur_dev )
    {
