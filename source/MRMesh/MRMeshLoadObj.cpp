...
void trimComment( std::string_view& line )
{
    ...
    //'Wavefront OBJ file: filename.obj'
    if ( line.starts_with( "Wavefront OBJ file: " ) )
    {
        line = line.substr( 0, 19 ); // 'Wavefront OBJ file:'
        return;
    }

    //'OBJ written from filename.obj'
    if ( line.starts_with( "OBJ written from " ) )
    {
        line = line.substr( 0, 16 ); // 'OBJ written from'
        return;
    }

    //'Generated by Sweet Home 3D - Sat Apr 04 21:52:12 AEDT 2026'
    if ( line.starts_with( "Generated by Sweet Home 3D - " ) )
    {
        line = line.substr( 0, 26 ); // 'Generated by Sweet Home 3D'
        return;
    }

    //'This file uses centimeters as units for non-parametric coordinates.' - units boilerplate, no software info
    if ( line.starts_with( "This file uses " ) || line.starts_with( "vertex coordinates are " ) )
    {
        line = std::string_view{};
        return;
    }
