...
if [ "${MR_EMSCRIPTEN}" == "ON" ]; then
  ...
  # side module, so wasm-ld wants every object in it PIC. 4.0.19 tolerated non-PIC archives,
  # 6.0.9 rejects them (libzip.so against libmbedcrypto.a, and libjpeg-turbo).
  MR_CMAKE_OPTIONS="${MR_CMAKE_OPTIONS} \
