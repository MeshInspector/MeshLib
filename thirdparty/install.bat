...
if errorlevel 1 (
    ...
    if !write_s3_option! equ true (
        set "VCPKG_BINARY_SOURCES=clear;x-aws,s3://vcpkg-export/fbba5251f75c149d71737fd3ef09629f41b5d5aa/x64-windows-meshlib/,readwrite;"
        echo "using aws auth"
    ) else (
        set "VCPKG_BINARY_SOURCES=clear;x-aws-config,no-sign-request;x-aws,s3://vcpkg-export/fbba5251f75c149d71737fd3ef09629f41b5d5aa/x64-windows-meshlib/,readwrite;"
        echo "using no auth"
    )
)
