...
if errorlevel 1 (
    ...
    if !write_s3_option! equ true (
        set "VCPKG_BINARY_SOURCES=clear;x-aws,s3://vcpkg-export/2024.09.30/x64-windows-meshlib/,readwrite;"
        echo "using aws auth"
    ) else (
        set "VCPKG_BINARY_SOURCES=clear;x-aws-config,no-sign-request;x-aws,s3://vcpkg-export/2024.09.30/x64-windows-meshlib/,readwrite;"
        echo "using no auth"
    )
)
