...
int main( void )
{
    ...
    MR_Image* image = MR_expected_MR_Image_std_string_value_mut( imageLoadRes );
    if ( !image )
    {
        fprintf( stderr, "Failed to load image: %s\n", MR_std_string_data( MR_expected_MR_Image_std_string_error( imageLoadRes ) ) );
    ...
    MR_DistanceMap* dm = MR_expected_MR_DistanceMap_std_string_value_mut( dmRes );
    if ( !dm )
    {
        fprintf( stderr, "Failed to convert image to distance map: %s\n", MR_std_string_data( MR_expected_MR_DistanceMap_std_string_error( dmRes ) ) );
