...
// zlibCompressStream uses zlib-ng in native mode (zng_ prefix, libz-ng).
// zlibDecompressStream stays on stock zlib. Both headers are included here
// because they share the Z_* return-code / flush-mode / strategy constants
// with identical values -- a redefinition with the same replacement list is
// allowed by the C preprocessor and does not produce a warning. The function
// names (deflate/inflate vs zng_deflate/zng_inflate) and struct types
// (z_stream vs zng_stream) are disjoint, so both libraries coexist cleanly.
#include <zlib.h>
#include <zlib-ng.h>
