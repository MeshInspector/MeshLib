...
# Add the PPA.
apt -y update && apt -y upgrade && apt -y install software-properties-common curl
add-apt-repository -y ppa:deadsnakes/ppa

# Install the packages.
apt -y install $(echo $PY_VERSIONS | perl -pe 's/(\S+)/python\1-dev python\1-venv/g')
...
for ver in $PY_VERSIONS; do
    if apt show python$ver-distutils >/dev/null 2>/dev/null; then
        apt -y install python$ver-distutils
    fi
done
