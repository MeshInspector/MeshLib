...
// zlibCompressStream(...) and its int-level overload are implemented in
// MRZlibNg.cpp against zlib-ng's native API (zng_* prefix). Decompression
// stays on stock zlib -- zlibDecompressStream has to accept bytes written
// by any deflate producer, so compatibility matters more than speed, and
// libzip still uses stock zlib as well.
