...
MRVIEWER_API void packPointsWithHistory( const std::shared_ptr<ObjectPoints>& objPoints );

/// sets new valid vertices then packs ObjectPoints optimally updating colors and adds history records for that;
/// it is a good idea to make SCOPED_HISTORY before calling this function
MRVIEWER_API void packPointsWithHistory( const std::shared_ptr<ObjectPoints>& objPoints, VertBitSet newValidVerts );
