...
def test_conversion_from_mrmesh(test_mesh_name, ext, use_fileHandler, tmp_path):
    ...
    if use_fileHandler:
        with open(input_folder / (test_mesh_name + ".mrmesh"), "rb") as mesh_handler:
            input_mesh = mrmeshpy.loadMesh(fileHandle=mesh_handler, extension="*.mrmesh")
    else:
        input_mesh = mrmeshpy.loadMesh(path=str(input_folder / (test_mesh_name + ".mrmesh")))
    # Save mesh
    ...
    if use_fileHandler:
        ...
        mrmeshpy.saveMesh(mesh=input_mesh, path=str(tmp_path / filename))
    # Comparing files
...
def test_conversion_to_mrmesh(test_mesh_name, ext, use_fileHandler, tmp_path):
    ...
    if use_fileHandler:
        with open(input_folder / (test_mesh_name + "." + ext), "rb") as mesh_file:
            input_mesh = mrmeshpy.loadMesh(fileHandle=mesh_file, extension="*." + ext)
    else:
        input_mesh = mrmeshpy.loadMesh(path=str(input_folder / (test_mesh_name + "." + ext)))
    # Saving mesh
