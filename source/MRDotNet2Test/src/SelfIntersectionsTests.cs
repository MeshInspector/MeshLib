...
namespace MRTest
{
    ...
    internal class SelfIntersectionsTests
    {
        ...
        public void TestSelfIntersections()
        {
            ...
            Assert.That( mesh.topology.getValidFaces().count(), Is.EqualTo((nuint)1024) );

            var intersections = SelfIntersections.getFaces(mesh);
            Assert.That( intersections.count, Is.EqualTo((nuint)128) );
            ...
            Assert.That(mesh.topology.getValidFaces().count(), Is.EqualTo((nuint)1194));

            intersections = SelfIntersections.getFaces(mesh);
            Assert.That(intersections.count, Is.EqualTo((nuint)0));
        }
    }
}
