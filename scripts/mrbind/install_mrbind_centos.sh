...
# Read the Clang version from `clang_version_centos.txt`. `xargs` trims the whitespace.
CLANG_VER="$(cat "$SCRIPT_DIR/clang_version_centos.txt" | xargs)"
...
CC=clang-$CLANG_VER CXX=clang++-$CLANG_VER cmake -B build -DCMAKE_EXPORT_COMPILE_COMMANDS=ON
