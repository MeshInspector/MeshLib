...
// Face incidence type:
// if PerEdge face can have neighbor only via edge
// if PerVertex face can have neighbor via vertex
enum FaceIncidence
{
    PerEdge,
    PerVertex
...
MRMESH_API UnionFind<VertId> getUnionFindStructureVertsSeparatedByPath( const Mesh& mesh, const SurfacePath& path, 
    VertBitSet * outPathVerts = nullptr ); //this set receives all vertices passed by the path
