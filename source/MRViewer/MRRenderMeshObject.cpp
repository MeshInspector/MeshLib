...
void RenderMeshObject::bindMesh_( bool alphaSort )
{
    ...
    if ( bool( dirty_ & DIRTY_TEXTURE ) )
    {
        ...
        pixels.reserve( size_t( res.x ) * res.y * textures.size() );
        for ( const auto& tex : textures )
            pixels.insert( pixels.end(), tex.pixels.begin(), tex.pixels.end() );
        GlTexture3::Settings settings;
    ...
    faceSelectionTex_.loadDataOpt( faceSelection.dirty(),
        { .resolution = texturePerFaceTextureSize_, .internalFormat = GL_R32UI, .format = GL_RED_INTEGER, .type = GL_UNSIGNED_INT },
        texturePerFaces );
