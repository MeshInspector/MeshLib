...
/// if absolute distance to the point is more than \param maxAbsDist, it will not be considered as shell vertex
[[nodiscard]] MRMESH_API bool isInnerShellVert( const MeshPart & mp, const Vector3f & shellPoint, Side side, float maxAbsDist = FLT_MAX );
...
/// Vertices with distances to the mesh more than \param maxAbsDist will not be considered as shell vertices
[[nodiscard]] MRMESH_API VertBitSet findInnerShellVerts( const MeshPart & mp, const Mesh & shell, Side side, float maxAbsDist = FLT_MAX );
