...
path_to_cmake_templates = os.path.join(os.path.join(it.base_path,'cmake'),'windows-dist')
path_to_cmake_config = os.path.join(os.path.join(os.path.join(it.path_to_install_folder,'lib'),'cmake'),'MeshLib')
...
vcpkg_triplet = sys.argv[2] if len(sys.argv) > 2 else "x64-windows-meshlib"
...
def copy_cmake_config():
	# The package ships no CMake config files of its dependencies, so `find_package(MeshLib)`
	# cannot use the ones generated by the CMake build: they call find_dependency() on all of
	# them. These are hand-written for the layout produced by this script.
	substitutions = {
		'@MESHLIB_ITERATOR_DEBUG_LEVEL@': '2' if 'iterator-debug' in vcpkg_triplet else '0',
		'@MESHLIB_VERSION@': sys.argv[1][1:] if len(sys.argv) > 1 else '0.0.0.0',
	}
	os.makedirs(path_to_cmake_config, exist_ok=True)
	for name in ['meshlib-config.cmake', 'meshlib-config-version.cmake']:
		content = open(os.path.join(path_to_cmake_templates, name + '.in'), 'r').read()
		for pattern, value in substitutions.items():
			content = content.replace(pattern, value)
		open(os.path.join(path_to_cmake_config, name), 'w').write(content)
...
it.copy_cmake_config = copy_cmake_config
