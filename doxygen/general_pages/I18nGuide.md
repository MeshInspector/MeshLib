# Internationalization Guide {#I18nGuide}
...
- **gettext tools**: `xgettext`, `msgmerge`, `msgfmt`. Install via your OS package manager or from vcpkg (`vcpkg install gettext[tools]`). Alternatively, set the `GETTEXT_ROOT` environment variable to a standalone gettext install path
  (e.g. <a href="https://github.com/vslavik/gettext-tools-windows/releases">gettext-tools-windows</a>).
...
`source/CompileTranslations.targets` provides an equivalent build-time `.po` → `.mo` compilation step for Visual Studio projects.
Import the targets file and declare one or more `TranslationLocaleDir` items pointing at your `locale/` root directories:
