# MeshLib overlay: mirror upstream vcpkg's `zlib-ng` port at the 2026.03.18
# tag (zlib-ng 2.3.3). Pinning here -- rather than relying on whatever
# vcpkg pin happens to be in effect -- avoids a regression specific to
# vcpkg 2024.10.21's zlib-ng port (zlib-ng 2.1.5#1):
#
#     CMake Error at scripts/cmake/vcpkg_replace_string.cmake:12 (file):
#       file failed to open for reading (No such file or directory):
#         .../zlib-ng_<triplet>/lib/pkgconfig/zlib.pc
#     ports/zlib-ng/portfile.cmake:41 (vcpkg_replace_string)
#
# Cause: the 2024.10.21 portfile forgets to pass
# `-DZLIB_COMPAT=${ZLIB_COMPAT}` to vcpkg_cmake_configure, so the actual
# zlib-ng cmake build always runs in native mode (producing zlib-ng.pc,
# libz-ng.dll). But its post-install fixup *does* read ZLIB_COMPAT from
# the triplet and looks for zlib.pc -- which the build never produced.
# 2026.03.18's port adds the missing -DZLIB_COMPAT line, which is what
# we mirror here so both vs2019 (vcpkg 2024.10.21) and vs2022
# (2026.03.18) build the same zlib-ng version with the same wiring.
