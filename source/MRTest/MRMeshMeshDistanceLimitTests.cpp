...
// clang-format off
// ----------------------------------------------------------------------------
// (c) TouchPoint - a sphere vertex sits on a cube face.
//
...
//   |  |       |
//   |  |       |          __
//   |  |       |         /  \      sphere b: r=0.5 at (2.5, 0, 0)
//  0|  |   a   |o------ ( b  )     pole vertex (2,0,0) lies on cube face x=2
//   |  |       |         \__/
//   |  |       |
// -2|  +-------+
//   +-------------------> x
//      -2      2  2.5  3
//
//  A UV sphere with verticalResolution=16 has a vertex at sphere-local
//  (-r, 0, 0) (theta=pi/2, phi=pi); shifting by (radius + 2, 0, 0) places
//  this vertex on a's face. The triangulation has finite resolution, so
//  distSq from findTriTriDistance is a few microns squared, not exactly 0.
// clang-format on
TEST( MRMesh, MeshMeshDistance_TouchPoint )
{
    const auto a = makeRefCube(); // [-2, 2]^3
    const float r = 0.5f;
    const auto b = makeUVSphere( r, 16, 16 ); // pole vertex at (-r, 0, 0)
    const AffineXf3f xf = AffineXf3f::translation( Vector3f( 2.0f + r, 0.0f, 0.0f ) );

    // tolerance is loose because the sphere vertex is computed via trig
    // and the edge-edge step in findTriTriDistance does not find the exact
    // touch point - actual distSq is on the order of 1e-5.
    constexpr float kTouchDistSqTol = 1e-3f;
    constexpr float kTouchDistTol = 1e-1f;

    // no limit: distSq is a few microns squared (not exactly 0), so the
    // projection-based status check runs. The sphere mesh as a whole sits
    // at x >= 2; the closest sphere point ends up on the +x side of the
    // cube face -> BothOutside.
    {
        const auto d = findDistance( a, b, &xf, kNoLimitSq );
        EXPECT_NEAR( d.distSq, 0.0f, kTouchDistSqTol );

        const auto sd = findSignedDistance( a, b, &xf, kNoLimitSq );
        EXPECT_NEAR( sd.signedDist, 0.0f, kTouchDistTol );
        EXPECT_EQ( sd.status, MeshMeshCollisionStatus::BothOutside );
    }

    // 1 mm limit: real distance is essentially 0 -> within the limit, behaves
    // exactly like the no-limit case.
    {
        const auto d = findDistance( a, b, &xf, kOneMmLimitSq );
        EXPECT_NEAR( d.distSq, 0.0f, kTouchDistSqTol );
    ...
    // strict 0 limit: findDistance always reports 0 with no exploration.
    // findCollidingTriangles is empty for a sphere just touching a cube face
    // (the sphere mesh stays at x >= 2 and only the pole vertex sits on the
    // cube face plane), so status = Touching.
    {
        const auto d = findDistance( a, b, &xf, kZeroLimitSq );
        EXPECT_FLOAT_EQ( d.distSq, kZeroLimitSq );
