...
- **On macOS**, the Viewer must run on the main thread of the process, which is where your Python script runs. So put the code driving the Viewer into a function and pass it as `mrviewerpy.launch(script=main)`: the window runs on the main thread, `main` on a worker thread, and `launch()` returns once `main` has. The [**Viewer example**](\ref ExampleViewer) is written this way and runs on every platform. The plain `mrviewerpy.launch()` followed by viewer calls raises a `RuntimeError` on macOS (releases before 3.1.3.566 terminated the Python process outright, with no exception).
