...
bool Viewer::loadFiles( const std::vector<std::filesystem::path>& filesList, const FileLoadOptions & options )
{
    ...
    const auto postProcess = [this, options] ( const SceneLoad::SceneLoadResult& result )
    {
        if ( result.scene )
        {
            ...
            if ( !options.forceObjectName.empty() && result.scene->children().size() == 1 && result.scene->children().front()->children().empty() )
                result.scene->children().front()->setName( options.forceObjectName );
