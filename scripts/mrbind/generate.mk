...
$($1__CombinedHeaderOutput): $($1__InputFiles) | $(TEMP_OUTPUT_DIR)
	...
	$(if $(is_py),\
		$(call,### Pull in defines for macros whose values are string literals containing backslashes [most notably `MB_PB11_ADJUST_NAMES`, whose value `"s/\\bMR:::/g"` carries a regex word-boundary `\b`]. Routing such values via `-D` through the make+bash recipe pipeline is fragile: on some MSYS2 environments [confirmed: the runner's preinstalled C:\msys64], the value reaches clang's PCH-build and per-fragment-build recipes inconsistently, producing `definition of macro 'MB_PB11_ADJUST_NAMES' differs between the precompiled header ('"s/\bMR:://g"') and the command line ('"s/\\bMR:://g"')`. The same clang 18.1.8 + same `-D` flag works correctly on master's frozen S3 MSYS2 snapshot, so the trigger is something in newer bash/make/coreutils. Defining the macro in a header that's part of the PCH source side-steps the `-D`-flag round-trip — clang stores and validates the same C-source spelling on both ends. Skipped during the mrbind parser pass [which doesn't have scripts/mrbind/ on its include path and doesn't need these macros anyway], same pattern as `<pybind11/pybind11.h>` below.)\
