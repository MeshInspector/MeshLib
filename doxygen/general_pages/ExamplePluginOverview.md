# C++ Example Plugin Overview {#ExamplePluginOverview}
...
> [!NOTE]
> Please have a look at [MeshLib С++ setup guide](\ref CppSetupWindows) first.
...
7. `CMakeLists.txt` - CMake project file
8. `ViewerApp.cpp` - sample viewer app that has enough functionality to load and test the tools
9. `wasm/index.html` - (WebAssembly only) sample HTML page for loading the sample viewer app
...
`example_plugin.vcxproj` - This file is configured so project can work with MeshLib distribuiton \include{doc} CommonDistributionProperties.md
...
`MyPlugin.items.json` - this file contains information about each tool that is present in the plugin:
 ...
 - **"Icon"** - unicode symbol from [Fontawesome](https://fontawesome.com), it is shown in UI for the tool if there is no actual icon present in `resource` folder
...
### Sample Viewer App
`ViewerApp.cpp` - source file for running the viewer instance. You can set your own
[viewer launch parameters](\ref LaunchParams) and alterate [viewer setup process](\ref ViewerSetup).
...
The system-wide alternative (writes into the notarized `MeshInspector.app` bundle) is documented under [How to Add Plugin](\ref HowtoAddPluginOverview) → macOS option B, including the signature trade-off.
...
3. Configure and build the project using CMake and emcmake:
```sh
...
> [!NOTE]
> Make sure your web server enables [cross-origin isolation](https://web.dev/articles/coop-coep) for these files.
...
```
the plugin's transitive dependencies didn't resolve from the host bundle (so the dylib never loaded, and therefore no items got registered). See [How to Add Plugin](\ref HowtoAddPluginOverview) → "Verifying the install" for fix paths.
