...
MRVIEWER_API bool buttonEx( const char* label, bool active, const Vector2f& size = Vector2f( 0, 0 ), ImGuiKey key = ImGuiKey_None, ImGuiButtonFlags flags = ImGuiButtonFlags_None );
/// draw gradient button, which can be disabled (active = false)
...
{
    return buttonEx( label, true, size, key );
}
