...
FORCE_REGISTER_RETURN_TYPE( std::function<Expected<void>( const SimpleVolumeMinMax& volume, int zOffset )> ); // For `IComputePointsToDistanceVolume::computeByParts`. Using `RETURN_TYPE` because this is a callback parameter, and in callback signatures "param" and "return" are swapped.
