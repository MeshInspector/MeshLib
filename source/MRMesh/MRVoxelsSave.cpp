...
tl::expected<void, std::string> saveAllSlicesToImage( const SavingSettings& settings )
{
    ...
    case SlicePlain::XY:
    {
        ...
        {
            const auto res = saveSliceToImage( settings.path / fmt::format( fmt::v8::runtime(settings.format), z, maxNumChars ), settings.vdbVolume, settings.slicePlain, z );
            if ( !res )
                return res;
    ...
    case SlicePlain::YZ:
    {
        ...
        {
            const auto res = saveSliceToImage( settings.path / fmt::format( fmt::v8::runtime( settings.format ), x, maxNumChars ), settings.vdbVolume, settings.slicePlain, x );
            if ( !res )
                return res;
    ...
    case SlicePlain::ZX:
    {
        ...
        {
            const auto res = saveSliceToImage( settings.path / fmt::format( fmt::v8::runtime( settings.format ), y, maxNumChars ), settings.vdbVolume, settings.slicePlain, y );
            if ( !res )
                return res;
