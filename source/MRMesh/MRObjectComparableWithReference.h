...
class MRMESH_CLASS ObjectComparableWithReference
{
  public:
    ...
    virtual void enableComparisonTolerances() = 0;

    // Returns the tolerance for a specific comparable property. Only call if `hasComparisonTolerances() == true`.
    [[nodiscard]] virtual ComparisonTolerance getComparisonTolerences( std::size_t i ) = 0;
    ...
    [[nodiscard]] bool isComparisonReferenceObject() const
    {
        return hasComparisonTolerances() && !hasComparisonReferenceValues();
    }
};
