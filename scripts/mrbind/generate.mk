...
$($1__CombinedHeaderOutput): $($1__InputFiles) | $(TEMP_OUTPUT_DIR)
	...
	$(if $(is_py),\
		$(call,### Pull in defines for macros whose values are string literals containing backslashes [most notably `MB_PB11_ADJUST_NAMES`, whose value `"s/\\bMR:::/g"` carries a regex word-boundary `\b`]. Clang's PCH validation re-renders such `-D` macro values when comparing the PCH against the new TU, and the re-rendered form drops a backslash, producing `definition of macro 'MB_PB11_ADJUST_NAMES' differs between the precompiled header ('"s/\bMR:://g"') and the command line ('"s/\\bMR:://g"')`. Defining the macro in a header that's part of the PCH source side-steps the `-D`-flag round-trip — clang stores and validates the same C-source spelling on both ends. Skipped during the mrbind parser pass [which doesn't have scripts/mrbind/ on its include path and doesn't need these macros anyway], same pattern as `<pybind11/pybind11.h>` below.)\
