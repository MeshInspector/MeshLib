...
MRCUDA_API Expected<std::vector<MR::PointsProjectionResult>> findProjectionOnPoints( const PointCloud& pointCloud,
    const std::vector<Vector3f>& points, const AffineXf3f* pointsXf = nullptr, const AffineXf3f* refXf = nullptr,
    float upDistLimitSq = FLT_MAX, float loDistLimitSq = 0, bool skipSameIndex = false );
